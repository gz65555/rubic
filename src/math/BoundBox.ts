...
export class BoundBox implements IClone {
   ...
   * @internal
   */
  private _rotateExtents(extents: Vector3, rotation: Matrix4, out: Vector3): void {
    ...
    out.x =
      Math.abs(matElements[0] * extentsX) + Math.abs(matElements[4] * extentsY) + Math.abs(matElements[8] * extentsZ);
    out.y =
      Math.abs(matElements[1] * extentsX) + Math.abs(matElements[5] * extentsY) + Math.abs(matElements[9] * extentsZ);
    out.z =
      Math.abs(matElements[2] * extentsX) + Math.abs(matElements[6] * extentsY) + Math.abs(matElements[10] * extentsZ);
  }
   ...
   * @internal
   */
  tranform(matrix: Matrix4, out: BoundBox): void {
