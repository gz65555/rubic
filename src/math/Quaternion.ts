...
export class Quaternion implements IClone {
	 ...
	 * @param	out  输出四元数
	 */
	static createFromMatrix4x4(mat: Matrix4, out: Quaternion): void {
	...
	getYawPitchRoll(out: Vector3): void {
		...
		if (angle.x == Math.PI / 2) {
			...
			Matrix4.createRotationY(-angle.y, Matrix4.TEMPMatrix0);
			Matrix4.createRotationX(-angle.x, Matrix4.TEMPMatrix1);

			Vector3.transformCoordinate(Quaternion.TEMPVector32, Matrix4.TEMPMatrix0, Quaternion.TEMPVector32);
			Vector3.transformCoordinate(Quaternion.TEMPVector32, Matrix4.TEMPMatrix1, Quaternion.TEMPVector32);
	...
	static lookAt(eye: Vector3, target: Vector3, up: Vector3, out: Quaternion): void {
		Matrix3.lookAt(eye, target, up, Quaternion._tempMatrix3x3);
	 ...
	 * @param	out        四元数
	 */
	static rotationMatrix(matrix3x3: Matrix3, out: Quaternion): void {
