...
import { Matrix4 } from "../math/Matrix4x4";
import { Quaternion } from "../math/Quaternion";
import { Vector3 } from "../math/Vector3";
import { Matrix3 } from "../math/Matrix3x3";
...
export class Transform3D {
  ...
  /** @internal */
  private static _tempMatrix0: Matrix4 = new Matrix4();
  /** @internal */
  private static _tempMatrix3x30: Matrix3 = new Matrix3();
  /** @internal */
  private static _tempMatrix3x31: Matrix3 = new Matrix3();
  /** @internal */
  private static _tempMatrix3x32: Matrix3 = new Matrix3();
  /** @internal */
  private static _tempMatrix3x33: Matrix3 = new Matrix3();
  ...
  set localPosition(value: Readonly<Vector3>) {
   ...
   * 局部旋转。
   */
  get localRotation(): Readonly<Quaternion> {
  ...
  set localRotation(value: Readonly<Quaternion>) {
  ...
  set localScale(value: Readonly<Vector3>) {
   ...
   * 局部空间欧拉角。
   */
  get localRotationEuler(): Readonly<Vector3> {
  ...
  set localRotationEuler(value: Readonly<Vector3>) {
  ...
  get localMatrix(): Readonly<Matrix4> {
    if (this._getTransformFlag(Transform3D.TRANSFORM_LOCALMATRIX)) {
      Matrix4.createAffineTransformation(this._localPosition, this.localRotation, this._localScale, this._localMatrix);
  ...
  set position(value: Vector3) {
    if (this._parent != null) {
      var parentInvMat: Matrix4 = Transform3D._tempMatrix0;
  ...
  get rotation(): Quaternion {
    if (this._getTransformFlag(Transform3D.TRANSFORM_WORLDQUATERNION)) {
      ...
      //使用localRotation不使用_localRotation,内部需要计算
  ...
  get worldMatrix(): Readonly<Matrix4> {
    if (this._getTransformFlag(Transform3D.TRANSFORM_WORLDMATRIX)) {
      if (this._parent != null) Matrix4.multiply(this._parent.worldMatrix, this.localMatrix, this._worldMatrix);
  ...
  set worldMatrix(value: Readonly<Matrix4>) {
    if (this._parent === null) {
      ...
      Matrix4.multiply(this._localMatrix, value, this._localMatrix);
    }
  ...
  private _getScaleMatrix(): Matrix3 {
    ...
    var invRotationMat: Matrix3 = Transform3D._tempMatrix3x30;
    var worldRotScaMat: Matrix3 = Transform3D._tempMatrix3x31;
    var scaMat: Matrix3 = Transform3D._tempMatrix3x32;
    Matrix3.createFromMatrix4x4(this.worldMatrix, worldRotScaMat);
    this.rotation.invert(invRotation);
    Matrix3.createRotationQuaternion(invRotation, invRotationMat);
  ...
  private _onWorldPositionScaleTransform(): void {
    if (
      ...
      this._setTransformFlag(
        Transform3D.TRANSFORM_WORLDMATRIX | Transform3D.TRANSFORM_WORLDPOSITION | Transform3D.TRANSFORM_WORLDSCALE,
        true,
      );
      // this.event(Event.TRANSFORM_CHANGED, this._transformFlag);
      for (var i: number = 0, n: number = this._children.length; i < n; i++)
  ...
  private _onWorldRotationTransform(): void {
    if (
      ...
      this._setTransformFlag(
        Transform3D.TRANSFORM_WORLDMATRIX | Transform3D.TRANSFORM_WORLDQUATERNION | Transform3D.TRANSFORM_WORLDEULER,
        true,
      );
      // this.event(Event.TRANSFORM_CHANGED, this._transformFlag);
      for (var i: number = 0, n: number = this._children.length; i < n; i++)
  ...
  _onWorldTransform(): void {
    if (
      ...
      // this.event(Event.TRANSFORM_CHANGED, this._transformFlag);
      for (var i: number = 0, n: number = this._children.length; i < n; i++) this._children[i]._onWorldTransform();
    }
  }
  ...
  translate(translation: Vector3, isLocal: boolean = true): void {
    if (isLocal) {
      Matrix4.createFromQuaternion(this.localRotation, Transform3D._tempMatrix0);
  ...
  setWorldLossyScale(value: Vector3) {
    if (this._parent !== null) {
      var scaleMat: Matrix3 = Transform3D._tempMatrix3x33;
      var localScaleMat: Matrix3 = Transform3D._tempMatrix3x33;
      var localScaleMatE: Float32Array = localScaleMat.elements;
      var parInvScaleMat: Matrix3 = this._parent._getScaleMatrix();
      parInvScaleMat.invert(parInvScaleMat);
      Matrix3.createFromScaling(value, scaleMat);
      Matrix3.multiply(parInvScaleMat, scaleMat, localScaleMat);
